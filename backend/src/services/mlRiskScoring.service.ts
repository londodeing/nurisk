...
export interface RiskFeatures {
  // Historical features
  historicalFrequency: number; // Incidents in last 30 days
  historicalSeverity: number; // Average severity
  recurrenceRate: number; // Days since last incident
  ...
  weatherAlert: number; // 0-4 alert level

  // Geographic features
  elevation: number; // meters
  slopeAngle: number; // degrees
  distanceToRiver: number; // km
...
export class MLRiskScoringService {
   ...
   */
  async calculateRiskScore(features: Partial<RiskFeatures>): Promise<RiskPrediction> {
    ...
    for (const [name, value] of Object.entries(featureVector)) {
      const weight = this.weights[name as keyof typeof this.weights] || 0;
   ...
   */
  private extractFeatures(features: Partial<RiskFeatures>): Record<string, number> {
    return {
   ...
   */
  private calculateConfidence(features: Partial<RiskFeatures>): number {
    let available = 0;
    let total = 0;

    // Historical features
    if ((features.historicalFrequency ?? -1) >= 0) available++;
    if ((features.historicalSeverity ?? -1) >= 0) available++;
    if ((features.recurrenceRate ?? -1) >= 0) available++;
    total += 3;

    // Weather features
    if ((features.rainfallIntensity ?? -1) >= 0) available++;
    if ((features.windSpeed ?? -1) >= 0) available++;
    if ((features.humidity ?? -1) >= 0) available++;
    if ((features.temperature ?? -1) >= 0) available++;
    if ((features.weatherAlert ?? -1) >= 0) available++;
    total += 5;

    // Geographic features
    if ((features.elevation ?? -1) >= 0) available++;
    if ((features.slopeAngle ?? -1) >= 0) available++;
    if ((features.distanceToRiver ?? -1) >= 0) available++;
    if ((features.distanceToCoast ?? -1) >= 0) available++;
    if ((features.populationDensity ?? -1) >= 0) available++;
    if ((features.vulnerabilityScore ?? -1) >= 0) available++;
    total += 6;

    // Source features
    if ((features.sourceReliability ?? -1) >= 0) available++;
    if ((features.sourceCount ?? -1) >= 0) available++;
    if ((features.verificationStatus ?? -1) >= 0) available++;
    total += 3;
   ...
   */
  async fetchFeatures(lat: number, lng: number): Promise<Partial<RiskFeatures>> {
   ...
   */
  private async fetchGeographicData(
    lat: number,
    lng: number
  ): Promise<Partial<RiskFeatures>> {
    try {
      ...
      const riverResult = await pool.query(`
        SELECT ST_Distance(
          ST_SetSRID(ST_MakePoint($1, $2), 4326)::geography,
      ...
      // Distance to coast
      const coastResult = await pool.query(`
        SELECT ST_Distance(
          ST_SetSRID(ST_MakePoint($1, $2), 4326)::geography,
   ...
   */
  async trainModel(incidentId: string, actualSeverity: number): Promise<void> {
    ...
    this.logger.log(
      `Model update: incident ${incidentId}, error ${error}, adjusting weights`
    );
   ...
   */
  private async fetchIncidentFeatures(
    incidentId: string
  ): Promise<Partial<RiskFeatures>> {
